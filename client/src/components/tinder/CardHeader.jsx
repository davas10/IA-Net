import React from 'react'
import profilePic from '../../assets/pruebas/profile-pic.avif'
import logo from '../../assets/tinder/logo.png'
import { IoIosNotifications } from 'react-icons/io'
import { useContext } from 'react'
import { useStateContext } from '../../context'


const style = {
  wrapper: `flex items-center bg-white w-full h-20 p-8 justify-evenly`,
  profileImage: `object-cover rounded-full`,
  logo: `object-contain`,
  notificationIcon: `sm:visible invisible text-3xl cursor-pointer text-gray-400 absolute`,
  notifications: `h-2 w-2 flex rounded-full relative bg-red-500 -top-3 -right-5`,
}



const CardHeader = () => {
  const { currentUserProfile } = useStateContext();

  return (
    <div className={style.wrapper}>
      <img
        src={currentUserProfile? currentUserProfile : profilePic}
        width={40}
        height={40}
        alt='profile-pic'
        className={style.profileImage}
      />
      <img
        src={logo}
        height={150}
        width={200}
        alt='logo'
        className={style.logo}
      />
      <div className='flex items-center'>
        <IoIosNotifications className={style.notificationIcon} />
        <div className={style.notifications} />
      </div>
    </div>
  )
}

export default CardHeader